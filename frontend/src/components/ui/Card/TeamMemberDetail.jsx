...
import {
  activateMember,
  deactivateMember,
} from "../../../api/differentApi's/members.api.js";
...
export default function TeamMemberDetail({
  ...
  const handleActivate = async () => {
    try {
      ...
      setSuccess(`${member.fullname} has been activated.`);
    } catch (err) {
          ...
          {member.role !== "admin" &&
            (member.isActive ? (
              ...
              <button
                ...
                className="w-full rounded-lg border border-emerald-200 py-2.5 text-sm font-medium text-emerald-600 hover:bg-emerald-50"
              >
                Activate member
              </button>
            ))}
